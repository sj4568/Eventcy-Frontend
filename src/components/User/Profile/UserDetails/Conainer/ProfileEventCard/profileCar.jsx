      ...
      <div className="card flex flex-row gap-4 p-2 h-60 border-none group relative f-card overflow-hidden">
        ...
        <div className="textBox">
            ...
            <div className="date flex text-sm font-semibold text-slate-600 lg:py-2 lg:place-items-center"> <FaFile/> {props.time} on {props.date} at {props.place}</div>
                      <div className="tickets text-red-500 font-bold pb-1 lg:py-3">Tickets from ${props.price} <span style={{ color: color }}> <select name="status" id="" onChange={AddValue}>
                      <option value={props.status==="open now"?"open now":"close now"}>{props.status==="open now"?"open now":"close now"}</option>
                      <option value={props.status==="open now"?"close now":"open now"}>{props.status==="open now"?"close now":"open now"}</option>
                      </select> </span></div>
            ...
            <div className="flex gap-2">
              ...
              <button className='border p-1 mt-2 px-3 text-md text-white rounded-lg bg-red-500 transition ease-in-out duration-500 hover:text-white' onClick={EventDelete}>Delete</button>
            </div>
        </div>
      </div>
    </div>
